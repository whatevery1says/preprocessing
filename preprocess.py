...
def preprocess_dir(manifest_dir, content_property, kwargs=options):
    ...
    for file in all_files:
        if file.endswith('.json'):
            ...
            path = '/'.join(tmp[:-1])
            filename = tmp[-1]
            preprocess(path, filename, content_property, kwargs=None)
    # Print time to completion
    end = time.time()
    t = end - start
    print('Processed all files in ' + str(t) + ' seconds.')

def preprocess_file(manifest_dir, filename, content_property, kwargs=options):
    ...
    """
    preprocess(manifest_dir, filename, content_property, kwargs=None)

def preprocess(manifest_dir, filename, content_property, kwargs=None, add_properties=None, remove_properties=None):
...
def main(**kwargs):
    ...
    if manifest_file is not None:
        ...
        preprocess_file(manifest_dir, manifest_file, content_property, kwargs=options)
        print('Finished preprocessing file.')
    else:
        print('Starting preprocessing...')
        preprocess_dir(manifest_dir, content_property, kwargs=options)
