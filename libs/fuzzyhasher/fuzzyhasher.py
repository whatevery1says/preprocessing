...
class FuzzyHasher:
    """A configurable fuzzy hash wrapper with utility functions
    ...
    def add_hash_to_json(self, data, add_new=True, update_old=True):
        """Adds a fuzzy hash to a JSON string. Adding new hashes
    ...
    def _filter_alnum(self, longstring):
        """Convert a string to an alphanumeric sequence
        with no spaces. Higher performance than .join(), as per
        https://stackoverflow.com/a/38799620/7207622
        """
        return re.sub('\W+','', longstring)

    def _filter_baggify(self, txt):
        """Convert a string to a sorted list of tokesn split on spaces."""
